...
def process_image(image_path, output_image_folder, filename, min_leaf_size=500, threshold_value=128):
    ...
    return total_leaf_area_cm2

def main(input_folder_path, output_image_folder):
    os.makedirs(output_image_folder, exist_ok=True)

    df = pd.DataFrame(columns=['Filename', 'Leaf Area'])
    ...
    for filename in os.listdir(input_folder_path):
        ...
        leaf_area = process_image(image_path, output_image_folder, filename)
        new_row = {'Filename': filename, 'Leaf Area': leaf_area}
...
if __name__ == "__main__":
    ...
    parser.add_argument('input_folder', help='The path to the input images folder')
    parser.add_argument('output_folder', help='The path to the output images folder')
